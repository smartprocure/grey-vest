...
The `props` property accepts an array of prop definitions (or an object - see the section below on **shorthand forms**), which is passed to Storybook's Props component and finally rendered into the props table that appears on a story's docs page. A prop definition has the following shape (see Storybook's [PropDef](https://github.com/storybookjs/storybook/blob/next/lib/components/src/blocks/PropsTable/PropDef.ts) type for reference):
...
For convenience, our storybook configuration performs a couple of transformations on `props` (or `propGroups`) so we can write the definitions a little more succinctly. Specifically:

1. In addition to an array of `{ name, ...definition }` objects, `props` can also be an *object* of the form `{ [name]: definition }`
2. In addition to a `{ summary, detail }` object, `type` and `defaultValue` also support a *plain string value*
