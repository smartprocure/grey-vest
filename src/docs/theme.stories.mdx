...
GreyVest uses an 8-pixel grid for spacing. Instead of an array of valid spacing values, we provide a function in `theme.space` that multiplies its input by 8. This way, if we need more granularity than the 8px grid, we can simply pass decimal numbers to `theme.space` - though using increments smaller than 0.5 (4px) is **_strongly discouraged_**.
