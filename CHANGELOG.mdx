...
* Changes to **Tabs**/**TabList**:
  * New prop: the `transparent` flag renders the tabs with a transparent background
  * 🚨 **Tabs**' `TabsList` component prop has been renamed to `TabList`
  * The old styling for TabList has been preseved as the component-level variant **TabList.Classic**, and can be used with the Tabs component by overriding its **TabList** prop: `<Tabs TabList={TabList.Classic} />`
