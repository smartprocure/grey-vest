...
- **TableFooter** - incorporates Pager, page size controls, and page info
...
- 🚨 **Awaiter**'s `promise` prop has been renamed to `promiseFn` (for consistency with react-async)
- Changes to **Button**:
  - New variants: `primary`, `secondary`, `danger`, and `ghost` colors/stylings, and `compact` and `large` sizes
...
2. Rename all `promise` props on **Awaiter** components to `promiseFn`
3. Convert all `onClose` props on **Popover** components to `onChange` (the callback receives the open/closed state of the popover as its argument)
4. Replace all references to...
   - **TabList** with **TabsList**
   - **SpacedList** with **ColumnList**
   - **TextButton** with **IconButton**
   - **ErrorList** with **ErrorText**
   - **LinkButton** with **Link**
5. Remove all references to the **ButtonRadio**, **BarChart**, and **Style** components
6. Remove any CSS targeting `gv-` classNames
...
- Remove some unused imports (react-date-picker, react-outside-click-handler)
